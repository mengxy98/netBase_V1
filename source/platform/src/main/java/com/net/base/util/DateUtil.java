...
public class DateUtil {
	...
	private static final Collection DEFAULT_PATTERNS = Arrays
			.asList(new String[] { "yyyy-MM-dd HH:mm:ss", "yyyyMMdd HH:mm:ss",
					"yyyy-MM-dd", "yyyyMMdd", "EEE MMM d HH:mm:ss yyyy",
	...
	public static Date getUpDate(String nowDay, String format, int y, int m,
			int d, int h, int min, int s) {
		Date date = toDate(nowDay, format);

		return getUpDate(date, y, m, d, h, min, s);
	}
	...
	public static Date getDownDate(String nowDay, String format, int y, int m,
		...
		return getDownDate(date, y, m, d, h, min, s);
	}
	 ...
	 * @return Date
	 */
	public static Date getUpDate(Date date, int y, int m, int d, int h,
			int min, int s) {
		...
		calendar.add(Calendar.HOUR, -d);// 得到前一小时
		calendar.add(Calendar.MINUTE, -min);// 得到前一分钟
		calendar.add(Calendar.SECOND, -s);// 得到前一秒
	 ...
	 * @return Date
	 */
	public static Date getDownDate(Date date, int y, int m, int d, int h,
			int min, int s) {
	...
	/**
	 * 根据传入时间格式获取开始时间和结束时间
	 * @param time1（时间格式1，如：周，月，季度，半年，年）
	 * @param time2（时间格式2，如：上周，本周，1月，5月，第一季度，第三季度，上半年，下半年，本年等）
	 * @return  返回一个list，包括开始时间和结束时间，格式为String类型的字符串，
	 * 			开始时间为日期+"000000",如：20150914000000，
	 * 			结束时间为日期+"235959",如：20150918235959
	 */
	public static List getStartTimeAndEndTime(String time1, String time2){
		...
		if("周".equals(time1)){
		...
		}else if("月".equals(time1)){
			if("1月".equals(time2)){
				calendar.set(Calendar.MONTH, 0);
			}else if("2月".equals(time2)){
				calendar.set(Calendar.MONTH, 1);
			}else if("3月".equals(time2)){
				calendar.set(Calendar.MONTH, 2);
			}else if("4月".equals(time2)){
				calendar.set(Calendar.MONTH, 3);
			}else if("5月".equals(time2)){
				calendar.set(Calendar.MONTH, 4);
			}else if("6月".equals(time2)){
				calendar.set(Calendar.MONTH, 5);
			}else if("7月".equals(time2)){
				calendar.set(Calendar.MONTH, 6);
			}else if("8月".equals(time2)){
				calendar.set(Calendar.MONTH, 7);
			}else if("9月".equals(time2)){
				calendar.set(Calendar.MONTH, 8);
			}else if("10月".equals(time2)){
				calendar.set(Calendar.MONTH, 9);
			}else if("11月".equals(time2)){
				calendar.set(Calendar.MONTH, 10);
			}else if("12月".equals(time2)){
			...
			eTime = calendar.getTime();
		}else if("季度".equals(time1)){
			if("第一季度".equals(time2)){
				calendar.set(Calendar.MONTH, 0);
			}else if("第二季度".equals(time2)){
				calendar.set(Calendar.MONTH, 3);
			}else if("第三季度".equals(time2)){
				calendar.set(Calendar.MONTH, 6);
			}else if("第四季度".equals(time2)){
				calendar.set(Calendar.MONTH, 9);
			}
			calendar.set(Calendar.DAY_OF_MONTH, 1);
			sTime = calendar.getTime();
			...
			calendar.add(Calendar.DATE, -1);
			eTime = calendar.getTime();
		}else if("半年".equals(time1)){
			if("上半年".equals(time2)){
				calendar.set(Calendar.MONTH, 0);
			}else if("下半年".equals(time2)){
				calendar.set(Calendar.MONTH, 6);
			}
			calendar.set(Calendar.DAY_OF_MONTH, 1);
			sTime = calendar.getTime();
			...
			calendar.add(Calendar.DATE, -1);
			eTime = calendar.getTime();
		}else if("年".equals(time1) && "当年".equals(time2)){
			int year = calendar.get(Calendar.YEAR);
			calendar.clear(); 
		...
		startTime = sdf.format(sTime) + "000000";
		endTime = sdf.format(eTime) + "235959";
		list.add(startTime);
		list.add(endTime);
		return list;
	}
	...
	/**
	 * 获取东八区的格式为“yyyyMMddHHmmss”的当前时间
	 * @return
	 */
	...
	/**
	 * 获取东八区的格式为“yyyyMMddHHmmss”的当前时间-1天
	 * @return
	 */
