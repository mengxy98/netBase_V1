...
import com.net.base.dao.ibatis.ext.dialect.Dialect;
...
public class LimitSqlExecutor extends SqlExecutor {
	...
	public void executeQuery(RequestScope request, Connection conn, String sql,
		...
		int len = sql.trim().length(); // update by hezhenjun
										// 解决数据量大，查询不到数据问题，原因：if (sql.length()
										// !=
										// len)偶然情况下，原sql和添加完limit后sql长度相等，不执行后续语句造成。
