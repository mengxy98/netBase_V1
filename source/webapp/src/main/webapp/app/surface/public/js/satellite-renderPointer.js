    ...
    Satellite.prototype = {
       ...
       if(!this.panel){
        ...
        drawSegmentText(ctx,text,curr){
            ...
            var r = text - l*1000;
            var er='';
           if(r<10){er='00'+r;}else  if(r<100){er='0'+r;}else{er=''+r;}
            var text = 'K'+l+'+'+er;
        ...
        draw:function(prevPos,currPos,gap,carId){
         ...
         /*
        update author: victor yang
        update time:2016.9.9
        [7],[8]关键部分互换；  
        longitude,latitude 改成x,y;
        level 改成 L; 
        单次请求一个车返回多条数据只显示一条的信息？
        
       
       */
        datasToRenderList:function(datas){
            ...
            var gap = this.configs.precision / this.configs.rate;
        ...
        addRenderList:function(datas){
            ...
            var that = this;
            var renderSet= that.datasToRenderList(datas);
            
            Object.keys(renderSet).forEach(function(k){
