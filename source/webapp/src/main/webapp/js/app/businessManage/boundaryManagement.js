...
function getLineList(){
	$.ajax({
		url : Main.contextPath+"/boundaryManagermentController/getLineList.do",
		type : "post",
		async : false,
		dataType : "json",
		success : function(result) {
...
function getLineDesignList(){
	$.ajax({
		...
		success : function(result) {
			window.parent.existsUser(result);//判断用户是否已失效
			var html="<option value='0'></option>";
			$("#contianerDetailAdd #lid").append(html);
			$("#contianerDetailRevise #lid").append(html);
			$.each(result,function(index,map){
				var datas = "" + "<option value='" + map.id
				+ "'>" + map.linename + "</option>";
				$("#contianerDetailAdd #lid").append($(datas));
				$("#contianerDetailRevise #lid").append($(datas));
			});
		}
	});
}
...
function getDataList() {
	...
	if (userManagerment_1) {
		...
		userManagerment_1 = $('#datatable_col_reorder_roleManagement').dataTable({
							...
							"autoWidth" : true,
							"ajax" : {
								url : Main.contextPath+ "/boundaryManagermentController/getDataList.do",
								type: 'POST',
								data : function(d) {
									return $.extend({},d,{
...
function addData(){
	...
	var lid = $("#contianerDetailAdd #lid").val();
   ...
   if(linename == ""){
	   ...
	   $.ajax({
			url : Main.contextPath+"/boundaryManagermentController/addData.do",
			type : "post",
			data:{params:params},
			dataType : "json",
			success : function(result) {
...
function deleteData(){
	...
	if(datas.length == '1'){
		if(confirm('确定要删除该路线吗?')){
			var id = datas[0].id;
			$.ajax({                     
				url : Main.contextPath+"/boundaryManagermentController/deleteData.do",
				type : "post",
				data:{id:id},
				dataType : "json",
				success : function(result) {
...
function modifyData(){
	...
	if(datas.length == '1'){
        ...
        $("#contianerDetailRevise  #lid option:contains("+datas[0].lineDesignName+")").attr("selected","selected");
...
function saveModify(){
	...
	var lid = $("#contianerDetailRevise  #lid").val();
   ...
   if(linename == ""){
	   ...
	   $.ajax({
			url : Main.contextPath+"/boundaryManagermentController/modifyData.do",
			type : "post",
			data:{params:params},
			dataType : "json",
			success : function(result) {
