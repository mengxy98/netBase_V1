...
function showJTopoToobar(stage) {
	var toobarDiv = $('<div class="jtopo_toolbar" style="z-index: 1000;">')
			.html(
					''
...
/*
 * runPrefixMethod(this, "RequestFullScreen"); if (typeof window.screenX ===
 * "number") { var eleFull = canvas; eleFull.addEventListener("click",
 * function() { if (runPrefixMethod(document, "FullScreen") ||
 * runPrefixMethod(document, "IsFullScreen")) { runPrefixMethod(document,
 * "CancelFullScreen"); this.title = this.title.replace("退出", ""); } else if
 * (runPrefixMethod(this, "RequestFullScreen")) { this.title =
 * this.title.replace("点击", "点击退出"); } }); } else { alert("浏览器不支持"); }
 */